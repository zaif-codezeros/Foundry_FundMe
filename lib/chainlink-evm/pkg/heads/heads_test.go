...
func TestHeads_LatestHead(t *testing.T) {
	...
	heads := heads.NewHeadSet()
	require.NoError(t, heads.AddHeads(testutils.Head(100), testutils.Head(200), testutils.Head(300)))

	latest := heads.LatestHead()
	require.NotNil(t, latest)
	require.Equal(t, int64(300), latest.Number)

	require.NoError(t, heads.AddHeads(testutils.Head(250)))
	latest = heads.LatestHead()
	require.NotNil(t, latest)
	require.Equal(t, int64(300), latest.Number)

	require.NoError(t, heads.AddHeads(testutils.Head(400)))
	latest = heads.LatestHead()
	require.NotNil(t, latest)
	...
	require.NoError(t, heads.AddHeads(newerH400))
	latest = heads.LatestHead()
	require.NotNil(t, latest)
	require.Equal(t, int64(400), latest.Number)
...
func TestHeads_HeadByHash(t *testing.T) {
	...
	heads := heads.NewHeadSet()
	require.NoError(t, heads.AddHeads(testHeads...))
...
func TestHeads_AddHeads(t *testing.T) {
	...
	require.NoError(t, heads.AddHeads(testHeads[2:5]...))
	require.Equal(t, 6, heads.Count())
	...
	t.Run("Returns an error, if newHead create cycle", func(t *testing.T) {
		...
		require.EqualError(t, heads.AddHeads(cycleHead), "cycle detected: newHeads reference itself newHead(Head{Number: 1000, Hash: 0x00000000000000000000000000000000000000000000000000000000000003e8, ParentHash: 0x00000000000000000000000000000000000000000000000000000000000003e8})")
	})
}
...
func TestHeads_MarkFinalized(t *testing.T) {
	...
	require.NoError(t, heads.AddHeads(h0, h1, h1Uncle, h2, h2Uncle, h3, h4, h5))
	...
	require.NoError(t, heads.AddHeads(h0, h1, h2, h2Uncle, h3, h4, h5))
	...
	require.NoError(t, heads.AddHeads(h1))
	...
	require.NoError(t, heads.AddHeads(h2))
...
// BenchmarkSimulated_Backfill - benchmarks AddHeads & MarkFinalized as if it was performed by Tracker's backfill
